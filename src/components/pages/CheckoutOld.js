...
class Checkout extends Component {
    ...
    validateInput( e ){
        ...
        switch( target_input ) {
            ...
            case 'email':
                if( !validateEmail( value ) ) {
                    ...
                    const errorMatch = newErrors.find((error) => {
                        return error === this.state.fields.emailField.errorMsg;
                    });
                    if( !errorMatch ) {
                    ...
                    if(errorMatch) {
                        ...
                        this.setState({
                            ...
                            isFormValidated: (remainingErrors.length === 0) ? true : false 
                        });   
                    }
            ...
            case 'card_number':
                if( !validatePaymentCardNumber( value ) ) {
                    ...
                    // check if error already exists
                    const errorMatch = newErrors.find((error) => {
                        return error === this.state.fields.cardNumberField.errorMsg;
                    });
                    if( !errorMatch ) {
            ...
            case 'date':
                ...
                if( !validateDate(value) ) {
                    ...
                    // check if error already exists
                    var errorMatch = newErrors.find((error) => {
                        return error === this.state.fields.dateField.errorMsg;
                    });
                    if( !errorMatch ) {
        ...
        if( isFormValid( this.state.errors, this.state.fields ) ) {
            this.setState({
